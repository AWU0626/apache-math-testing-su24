...
public class MyFractions {
    ...
    public static Fraction addSub(Fraction f1, Fraction f2, boolean isAdd) {
        ...
        int d1 = ArithmeticUtils.gcd(
                f1.getDenominator(),
                f2.getDenominator());

        if (d1 == 1) {
            // result is ( (u*v' +/- u'v) / u'v')
            int uvp = ArithmeticUtils.mulAndCheck(
                    f1.getNumerator(),
                    f2.getDenominator());

            int upv = ArithmeticUtils.mulAndCheck(
                    f2.getNumerator(),
                    f1.getDenominator());
        ...
        // t = u(v'/d1) +/- v(u'/d1)
        BigInteger uvp = BigInteger.valueOf(f1.getNumerator()).multiply(BigInteger.valueOf(f2.getDenominator() / d1));
        BigInteger upv = BigInteger.valueOf(f2.getNumerator()).multiply(BigInteger.valueOf(f1.getDenominator() / d1));
        BigInteger t = null;
